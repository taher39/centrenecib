...
function RoseBloom() {
  // SVG rose that blooms then the screen splits
  return (
...
function HomePage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-rose-gradient">
      ...
      {/* Split curtains */}
      <motion.div initial={{ x: 0 }} animate={stage !== "bloom" ? { x: "-100%" } : { x: 0 }} transition={{ duration: 0.9, ease: [0.7, 0, 0.3, 1] }}
        className="absolute inset-y-0 left-0 z-10 w-1/2 bg-gradient-to-br from-primary/95 to-primary/80" />
      <motion.div initial={{ x: 0 }} animate={stage !== "bloom" ? { x: "100%" } : { x: 0 }} transition={{ duration: 0.9, ease: [0.7, 0, 0.3, 1] }}
        className="absolute inset-y-0 right-0 z-10 w-1/2 bg-gradient-to-bl from-primary/95 to-primary/80" />
            ...
            <motion.div key="form" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full">
              ...
              {mode === null && (
                <div className="grid gap-3">
                  ...
                  <Button size="lg" variant="outline" className="h-14 rounded-2xl text-base border-primary/30" onClick={() => setMode("new")}>
                    {t("client.newClient")}
                  </Button>
                </div>
              )}
              ...
              {mode === "code" && (
                <form className="grid gap-4 rounded-2xl bg-card p-6 shadow-soft" onSubmit={(e) => e.preventDefault()}>
                  <div className="grid gap-2">
                    <Label>{t("client.codeLabel")}</Label>
                    <Input inputMode="numeric" maxLength={10} placeholder="——————————" className="h-12 text-center tracking-[0.4em] text-lg" />
                  </div>
                  <Button size="lg" className="h-12 rounded-xl">{t("client.enter")}</Button>
              ...
              {mode === "new" && (
                <form className="grid gap-4 rounded-2xl bg-card p-6 shadow-soft" onSubmit={(e) => e.preventDefault()}>
                  <div className="grid gap-2"><Label>{t("common.name")}</Label><Input className="h-11" /></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="grid gap-2"><Label>{t("common.age")}</Label><Input type="number" min={10} max={99} className="h-11" /></div>
                    <div className="grid gap-2"><Label>{t("common.phone")}</Label><Input type="tel" className="h-11" /></div>
                  </div>
                  <Button size="lg" className="h-12 rounded-xl">{t("client.book")}</Button>
                  <button type="button" className="text-xs text-muted-foreground hover:text-foreground" onClick={() => setMode(null)}>← {t("common.back")}</button>
                </form>
              )}
            </motion.div>
